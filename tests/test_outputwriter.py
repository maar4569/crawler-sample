...
class TestOutputCsvWriter(unittest.TestCase):
    ...
    def test_error_outputfilename(self):
        self.assertRaises(Exception,lambda: self.csvwriter.output("",self._urls))

    def test_error_urllist(self):
        self._urls2= []
 
        self.assertRaises(Exception,lambda: self.csvwriter.output("./dummy.txt",self._urls2))

    def test_success_output(self):
        if os.path.exists(self._filename) : os.remove(self._filename)
        ret = self.csvwriter.output(self._filename,self._urls)
        self.assertEqual(ret , 0)

    #test Url2JsonWriter
    def test_json_error_outputfilename(self):
        self.assertRaises(Exception,lambda: self.jsonwriter.output("",self._urls))

    def test_json_error_urllist(self):
        self._urls2= []
 
        self.assertRaises(Exception,lambda: self.jsonwriter.output("./dummy.txt",self._urls2))

    def test_json_success_output(self):
        if os.path.exists(self._jsonfile) : os.remove(self._jsonfile)
        ret = self.jsonwriter.output(self._jsonfile,self._urls)
        self.assertEqual(ret , 0)
